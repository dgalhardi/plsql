...
SELECT JOB_TITLE, DEPARTMENT_NAME, LAST_NAME, START_DATE FROM JOB_HISTORY JOIN JOBS USING (JOB_ID) JOIN DEPARTMENTS 
...
(SELECT EMPLOYEE_ID FROM JOB_HISTORY)
--62.Display country name, city, and number of departments
...
68.Display the details of employees drawing the highest salary
in the department.
...
Commit;
End;
--2.Increase the salary of employee 115 based on the following conditions: If experience is more than 10 years, increase
--salary by 20% If experience is greater than 5 years, increase salary by 10% Otherwise 5% Case by Expression:
declare
v_exp number(2);
v_hike number(5,2);
begin
...
--3.Change commission percentage as follows for employee with ID = 150. If salary is more than 10000 then commission is
--0.4%, if Salary is less than 10000 but experience is more than 10 years then 0.35%, if salary is less than 3000 then
--commission is 0.25%. In the remaining cases commission is 0.15%.
declare
...
--13.Create a procedure that takes department ID and changes the manager ID for the department to the employee in the department with highest salary. 
--(Use Exceptions).
...
--14.Create a function that takes a manager ID and return the names of employees who report to this manager. The names must be returned as a string 
--with comma separating names.
...
--causes mutating problem. The solution to mutating problem iexplained at : Work around for mutating problem in Oracle Triggers. Please check it out.
--18.Whenever the job is changed for an employee write the following details into job history. Employee ID, old job ID, old
--department ID, hire date of the employee for start date, system date for end date. But if a row is already present for employee job history then 
...
--does the same.
 ...
 |Cliente|   0,1 _______________ 0,N |Compra|
 +idClient                       +idCompra
 +nome                           +idClient
 +email                          +dataCompra
 ...
 --Utilizando o INNER JOIN você terá como resultado de sua consulta somente os pares de cliente/compra, ou seja, 
 --os casos em que uma compra não está ligada a nenhum cliente, ou cliente que não possuem compras, não serão apresentados no resultado.
 --3.LEFT JOIN
 SELECT * FROM cliente LEFT JOIN compra ON idcliente
 --O LEFT JOIN traz todos os resultados da tabela mais à esquerda e o agrega ao seu valor correspondente das outras tabelas, caso existam. 
 --Ou seja, nesse caso a consulta retornaria todos os clientes e suas compras, caso existam. Caso não existam compras para essa cliente os 
 --campos relativos à tabela de compras ficariam em branco.
 --4.RIGHT JOIN
 SELECT * FROM cliente RIGHT JOIN compra ON idcliente
 --De forma análoga ao LEFT JOIN, o RIGHT JOIN traz todos os resultados da tabela mais à direita e o agrega ao seu valor correspondente 
 --das outras tabelas, caso existam. Ou seja, nesse caso a consulta retornaria todas as compras e, caso existam, seus clientes. Caso não 
 --existam clientes para essa compra os campos relativos à tabela de clientes ficariam em branco.
 --5.OUTER JOIN
 SELECT * FROM cliente OUTER JOIN compra ON idcliente
 --O comando OUTER JOIN, mesmo não sendo muito conhecido, pode ser bastante útil em alguns casos. Ele traria como resultado todos os 
 --clientes que não estão ligados à nenhuma compra.
